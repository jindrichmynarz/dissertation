...
The supplementary vocabulary can be used to qualify concepts from the main vocabulary, such as `MF09` meaning *"Using hovercraft"*.
There are 903 concepts in the supplementary vocabulary, which is organized in a flat structure.
...
The monohierarchical design may have cause conceptual duplication within distinct branches of the vocabulary.
...
CPV is published in tree (XML) or tabular (XLS) data formats, which may have encouraged the vocabulary's monohierarchical design by making it simpler to implement.
...
Polyhierachy may be sidestepped by creating associative links between similar concepts within different branches of the vocabulary’s hierarchical structure.
...
We experimented with link discovery tools to construct associative links within the vocabulary.
...
Its result is described with the Simple Knowledge Organization System (SKOS) ([Isaac, Summers, 2009](#Isaac2009)), plus Dublin Core Terms^[<http://dublincore.org/documents/dcmi-terms>] for metadata.
...
The transformation was originally orchestrated by a shell script, which was later replaced by a UnifiedViews pipeline.

The Czech public procurement registered mandates the use of the 2008 version of CPV since September 15, 2008.
...
Cells that would duplicate the values of the cells above were left empty in the source spreadsheet.
Therefore, we had to create the *"fill down blanks"* functionality to duplicate cell values in following directly adjacent empty cells.
